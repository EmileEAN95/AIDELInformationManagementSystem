...
namespace ColorPicker
{
    ...
    public class Hook
    {
        ...
        int m_hHook = 0;
        WinColorUtil.HookProc m_hookCallback;

        public void SetHook(bool enable)
        {
            if (enable && m_hHook == 0)
            {
                m_hookCallback = new WinColorUtil.HookProc(HookCallbackProc);
                Module module = Assembly.GetExecutingAssembly().GetModules()[0];
                m_hHook = WinColorUtil.SetWindowsHookEx(WinColorUtil.WH_KEYBOARD_LL, m_hookCallback, Marshal.GetHINSTANCE(module), 0);
                if (m_hHook == 0)
                {
            ...
            {
                WinColorUtil.UnhookWindowsHookEx(m_hHook);
                m_hHook = 0;
            }
        }
        int HookCallbackProc(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode < 0)
            {
                return WinColorUtil.CallNextHookEx(m_hHook, nCode, wParam, lParam);
            }
            else
            {
                //Marshall the data from the callback.
                WinColorUtil.KeyboardHookStruct hookstruct = (WinColorUtil.KeyboardHookStruct)Marshal.PtrToStructure(lParam, typeof(WinColorUtil.KeyboardHookStruct));

                if (OnKeyDown != null && wParam.ToInt32() == WinColorUtil.WM_KEYDOWN)
                {
                ...
                int result = 0;
                if (m_hHook != 0)
                    result = WinColorUtil.CallNextHookEx(m_hHook, nCode, wParam, lParam);
                return result;
            }
        }
    }
