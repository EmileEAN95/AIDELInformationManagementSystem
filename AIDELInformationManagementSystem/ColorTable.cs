...
namespace ColorPicker
{
    public class ColorTable : LabelRotate
    {
        ...
        const int COLS = 16;
        const int ROWS = 8;
        public void SelectFirstCustomColor()
        {
            SetIndex(NUM_OF_NON_CUSTOM_COLORS);
        }
        //public int Cols
        //{
        //    get { return m_cols; }
        ...
        public void RemoveExtraCustomColor()
        {
            int maxNumOfColors = COLS * ROWS;
            if (m_colors.Count > maxNumOfColors)
                m_colors.RemoveAt(maxNumOfColors);
        }
        public void SetCustomColor(Color col)
        {
            if (m_colors.Contains(col) == false)
            {
                m_colors.Insert(NUM_OF_NON_CUSTOM_COLORS, col);
                Invalidate(GetRectangle(m_colors.Count - 1));
            }

            RemoveExtraCustomColor();
        }
        public Color[] Colors
        {
            ...
            set
            {
                ...
                if (value.Length >= NUM_OF_NON_CUSTOM_COLORS)
                    m_colors = m_colors.GetRange(0, NUM_OF_NON_CUSTOM_COLORS);
                for (int i = m_colors.Count; i <= COLS * ROWS; i++)
                {
                    m_colors.Add(Color.White);
                }
            }
        ...
        int m_spacing = 3;
        int m_selindex = 0;
        const int NUM_OF_NON_CUSTOM_COLORS = (ROWS - 1) * COLS;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            base.OnPaint(e);
            int index = 0;
            int totalwidth = COLS * (m_fieldSize.Width + m_spacing);
            int totalheight = ROWS * (m_fieldSize.Height + m_spacing);
